...
import 'historis_item_selesai.dart';
...
class _AlamatListState extends State<AlamatList> {
  ...
  Future<void> fetchDataAlamatList() async {
    SharedPreferences preferences = await SharedPreferences.getInstance();
    _token = preferences.getString("token");
    try {
      ...
      final response = await http.post(
          ...
          body: body);
      ResponseAlamatList _data = responseAlamatListFromJson(response.body);
      ...
      setState(() {
        _hasMore = _data.addresses.length == _defaultPerPageCount;
      ...
      print("catch alamat list: " + e.toString());
      setState(() {
        _loading = false;
        _error = true;
      });
  ...
  Widget getBody() {
    if (_alamat.isEmpty) {
      ...
      return ListView.builder(
          itemCount: _alamat.length + (_hasMore ? 1 : 0),
          itemBuilder: (context, index) {
            if (index == _alamat.length - _nextPageThreshold) {
              fetchDataAlamatList();
            }
            if (index == _alamat.length) {
              if (_error) {
                return Center(
                    child: InkWell(
                  onTap: () {
