...
class _Historis_Item_BatalState extends State<Historis_Item_Batal> {
  ...
  //     Uri.parse("http://10.0.2.2:8000/api/admin/cities/$idcity/get"),
  //     body: body,
  //   );
  ...
  get_data() async {
    ...
    final response = await http.post(
      Uri.parse(
          "http://10.0.2.2:8000/api/contributor/pickup_orders/$orderid/get"),
      body: body,
    );
    final data = jsonDecode(response.body);
    ...
    setState(() {
      ...
      if (data['pickup_orders']['weighing_volume'] == null) {
        weighing_volume = "- Liter";
      } else {
        String weighing_volumeTemp =
            data['pickup_orders']['weighing_volume'].toString();
        weighing_volume = weighing_volumeTemp + " Liter";
      }
      postal_code = data['pickup_orders']['postal_code'];
      estimate_volume = data['pickup_orders']['estimate_volume'].toString();
      //weighing_volume = data['pickup_orders']['weighing_volume'].toString();
